...
def generate_pv_full_scene(script_path="view_simulation.py"):
    """
    Visualization Agent v3 (MMC): Generates a complete Paraview Scene Script.
    Layers:
    1. Anatomy: Transparent Scalp + Skull + Brain.
    2. Optodes: Source (Red) + Detectors (Green) Array.
    3. Pulse: Volumetric Fluence from MMC (log_fluence_mmc).
    """
    
    cwd = os.getcwd()
    path_anat = os.path.join(cwd, "kf_anatomy_centered.xdmf")
    path_pulse = os.path.join(cwd, "kf_mmc_centered.xdmf")
    path_optodes = os.path.join(cwd, "kf_array_hardware.xdmf")
    
    script_content = f"""
from paraview.simple import *

#### 1. Load Anatomy (Context) ####
print("Loading Anatomy: {path_anat}")
anat = OpenDataFile('{path_anat}')
...
print("Loading Optodes: {path_optodes}")
optodes = OpenDataFile('{path_optodes}')
...
print("Loading Pulse: {path_pulse}")
pulse = OpenDataFile('{path_pulse}')
...
"""
    ...
    print(f"Visualization Agent: Generated '{script_path}' for full scene.")

if __name__ == "__main__":
    generate_pv_full_scene()
